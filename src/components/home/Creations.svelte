<script lang="ts">
	...
	function handleWheel(event: WheelEvent) {
		console.log({$windowSmoothScroll, event});
		$windowSmoothScroll.scrollTo($windowSmoothScroll.animatedScroll + $windowSmoothScroll.options.wheelMultiplier * 2 * event.deltaY);
	}
	function handleTouch(event: TouchEvent) {
		console.log({$windowSmoothScroll, event});
		// $windowSmoothScroll.scrollTo($windowSmoothScroll.animatedScroll + $windowSmoothScroll.options.wheelMultiplier * 2 * event.changedTouches[0].);
	}
</script>

<button on:click={() => { carouselIndex++}}>+</button>
<button on:click={() => { carouselIndex--}}>-</button>
<div class="-container overflow-x-scroll w-[1000px] max-w-full" data-lenis-prevent bind:this={scrollContainer} on:wheel={handleWheel} on:touchmove={handleTouch}>
	<div class="-carousel inline-flex gap-12 overflow-x-scroll" bind:this={scrollContent}>
		{#each currentData as el, index}
			<a href={el.url} class="-creation-{index} -creation-pos-{Math.abs(index - ((currentData.length - 1) / 2))}">
				<div class="-picture w-[18rem] max-w-[85vw] aspect-[2/3] bg-center bg-no-repeat bg-cover rounded-[4px]" style="background-image: url({el.img});">
				</div>
			</a>
		{/each}
	</div>
