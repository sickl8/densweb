 ...
 */
	...
	set fftSize( value ) {
		...
		// console.log({fftSize: value, binCount});
		this._fftDataFloat = [ new Float32Array( binCount ), new Float32Array( binCount ) ];
		this._fftDataInt = [ new Uint8Array( binCount ), new Uint8Array( binCount ) ];
		this._calcBars();
	}
	...
	connectInput( source ) {
		...
		if ( ! this._inputNodes.includes( node ) ) {
	...
	disconnectInput( sources, stopTracks ) {
		if ( ! sources )
			sources = Array.from( this._inputNodes );
		else if ( ! Array.isArray( sources ) )
			sources = [ sources ];

		for ( const node of sources ) {
			const idx = this._inputNodes.indexOf( node );
	...
	getEnergy( startFreq, endFreq ) {
		...
		for ( let channel = 0; channel < chnCount; channel++ ) {
			for ( let i = startBin; i <= endBin; i++ )
				energy += this._normalizedB( this._fftDataInt[ channel ][ i ] );
		}
...
*/
			...
			// console.log({fftData})
			// console.log({
			// 	fftDataMinNotInf: fftData.reduce((p, c) => {
			// 		return c === -Infinity ? p : Math.min(p, c)
			// 	} ,0).toFixed(4),
			// 	fftDataMax: fftData.reduce((p, c) => {
			// 		return Math.max(p, c)
			// 	} ,-Infinity).toFixed(4),
			// 	fftDataIntMinNotInf: fftDataInt.reduce((p, c) => {
			// 		return c === -Infinity ? p : Math.min(p, c)
			// 	} ,0).toFixed(4),
			// 	fftDataIntMax: fftDataInt.reduce((p, c) => {
			// 		return Math.max(p, c)
			// 	} ,-Infinity).toFixed(4),
			// 	nBars: nBars,
			// });
			...
			for ( let barIndex = 0; barIndex < nBars; barIndex++ ) {
				...
				// console.log({ bi: barIndex, freq, binLo, binHi, ratioLo, barValue })
